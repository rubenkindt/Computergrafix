...
GLfloat kuipCtrlPunten[6][4][3] =
{
   ...
   { {0.0, 0.0, 0.16}, {0.04, 0.0, 0.18}, {0.08, 0.0, 0.18}, {0.12, 0.1, 0.18}},
...
//------------------------------ondersteuning------------------------------
    ...
    glPushMatrix();
        ...
        gluCylinder(ondersteuning, 0.01, 0.01, 0.4, 8, 8);
    glPopMatrix();
...
//------------------------------dak------------------------------
    ...
    for(int i=0;i<4;i++){//dak genereren
        for(int j=0;j<4;j++){
    ...
    }else{  // gluCylinder geeft een mooier beeld dan cone
        GLUquadricObj *dak;
        dak = gluNewQuadric();
        glPushMatrix();
...
void torussen(GLfloat offsetX){
    ...
    glPushMatrix(); //torus1
        ...
        glTranslatef(2.5, 1.2, offsetX+2.2);
        glRotated(hoek, 0, 0, 1);
        if(tooncomplex)
    ...
    glPushMatrix(); //torus2
        ...
        glScalef(1.0, 1.0, 1.0);
        glTranslatef(2.5, 1.2, offsetX+2.8);
        glRotated(hoek, 0, 0, 1);
                if(tooncomplex)
            glutWireTorus(0.1, 1.6, 15, 15);
        else
...
void spaken(GLfloat offsetX){
    ...
    for(float i=0.0;i<360.0;i+=(360.0/cabines)){
        ...
        glPopMatrix();
        glPushMatrix();//spaak 2
            glTranslatef(2.5, 1.2, offsetX+2.25);
            glRotatef(-90.0, 1.0, 0.0, 0.0);
            glRotatef(i, 0.0, 1.0, 0.0);
            glRotated(-hoek, 0, 1, 0);
            if (tooncomplex)
...
void dwarsEnCabine(GLfloat offsetX){
    GLUquadricObj *dwars;
    dwars = gluNewQuadric();
...
void toets(unsigned char key, int x, int y)
{
    ...
    {   case 'X' : xoog--; break;
        ...
        case '4' : xoog =-1.0, yoog = 2.0, zoog = 7.0;  break;
        ...
        case '9' : xoog =-1.0, yoog = 2.0, zoog = 3.0;  break;
...
void display(void)
{
    ...
    for (int i=0;i>(-reuzenraderen);i--){
        steunbalken(4*i+0.0);   // elke ruezenrad liefst 4 uit ’’n
