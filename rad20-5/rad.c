...
void display(void)
{
    ...
    for (int i=0;i>(-reuzenraderen);i--){
        steunbalken(4*i+0.0);   // elke reuzenrad liefst 4 uit één
        as(4*i+0.0);    // reuzenrad is3 breed, mooier met 4 tussen
        spaken(4*i+0.0);
        torussen(4*i+0.0);
    ...
    glutSwapBuffers();// één voor tekenen andere voor het berekenen
    glDisable(GL_LIGHTING);
    glFlush();
}
int main(int argc, char *argv[]){
    ...
    glutCreateWindow("Kermis van de Nayer: Ruben Kindt");
    init();
